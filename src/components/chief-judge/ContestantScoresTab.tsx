...
interface ContestantScoresTabProps {
  competitionId: string;
  subEventId: string;
  allScores: JudgeScore[] | undefined;
}

export function ContestantScoresTab({ competitionId, subEventId, allScores }: ContestantScoresTabProps) {
  ...
  const contestants = useMemo(
    () => (registrations || [])
      .filter((r: any) => r.sub_event_id === subEventId && r.status === "approved")
  ...
  if (regsLoading) return <CardGridSkeleton cards={3} />;

  if (contestants.length === 0) {
    return (
      <Card className="border-border/50 bg-card/80">
        <CardContent className="py-8 text-center text-muted-foreground text-sm">
          No approved contestants for this sub-event.
        </CardContent>
      </Card>
    );
  }
          ...
          <Table>
            ...
            <TableBody>
              {pagedContestants.map((c: any) => {
                ...
                const isExpanded = expandedContestant === c.id;
                return (
                  <Fragment key={c.id}>
                    <TableRow
                      className="cursor-pointer hover:bg-muted/50 transition-colors"
                      onClick={() => setExpandedContestant(isExpanded ? null : c.id)}
                    >
