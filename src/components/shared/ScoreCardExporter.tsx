...
import { ScoreCard, ScoreCardBatch } from "./ScoreCard";
  ...
  const handleIndividualExport = async (contestant: ContestantRegistration) => {
    ...
    try {
      ...
      await exportElementAsPDF(
        scoreCardElement as HTMLElement,
        filename,
        {
          format: 'letter',
          orientation: 'landscape'
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2">
          {exportType === 'batch' ? (
            <Button
              ...
              disabled={isExporting || contestants.length === 0}
            ...
            <div className="text-sm text-muted-foreground">
              Individual exports available in the contestant list below
            </div>
          )}
            ...
            <div ref={batchRef} className="flex flex-wrap gap-2 justify-center">
              ...
              {contestants.length > 6 && (
                <div className="w-full text-center text-sm text-muted-foreground mt-4">
                  ... and {contestants.length - 6} more cards
                </div>
              )}
            </div>
          ) : (
            <div className="space-y-4">
              {contestants.slice(0, 3).map((contestant) => (
                <div key={contestant.id} className="flex items-center justify-between p-2 border rounded">
                  ...
                  <Button
                    size="sm"
                    onClick={() => handleIndividualExport(contestant)}
                    disabled={isExporting}
                  >
                    <Download className="h-4 w-4 mr-1" />
                    Export Card
                  </Button>
                  {/* Hidden score card for individual export */}
              ...
              {contestants.length > 3 && (
                <div className="text-center text-sm text-muted-foreground">
                  ... and {contestants.length - 3} more contestants
                </div>
              )}
            </div>
          )}
        ...
        <div className="text-sm text-muted-foreground space-y-1">
          <p><strong>Batch Export:</strong> Downloads all score cards as a single PDF file, perfect for printing multiple cards at once.</p>
          <p><strong>Individual Export:</strong> Downloads each score card as a separate PDF file.</p>
          <p><strong>Blank Templates:</strong> Uncheck "Include contestant data" to export blank score cards for manual filling.</p>
          <p><strong>Print Settings:</strong> Score cards are designed for 8" x 5" paper. Use landscape orientation for best results.</p>
        </div>
      </CardContent>
    </Card>
  );
