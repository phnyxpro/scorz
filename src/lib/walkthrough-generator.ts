import { helpArticles, HelpArticle } from '../content/help-articles';
...
interface WalkthroughSection {
  title: string;
  content: string;
  subsections?: WalkthroughSection[];
}
 ...
 * This function generates comprehensive markdown walkthroughs from the structured help articles data
 */
export function updateWalkthroughs(): void {
  ...
  const walkthroughConfigs: Record<string, WalkthroughConfig> = {
    judge: {
  ...
  Object.entries(articlesByCategory).forEach(([categorySlug, articles]) => {
    ...
    const filePath = join(process.cwd(), 'content', 'walkthroughs', fileName);
 ...
 */
function generateWalkthroughMarkdown(config: WalkthroughConfig, tocItems: string): string {
...
/**
 * Watch for changes and auto-update walkthroughs
 * This function can be called during build process or development
 */
export function watchAndUpdateWalkthroughs(): void {
  // This could be enhanced to watch for file changes
  // For now, just run the update
  updateWalkthroughs();
}

/**
 * Validate walkthrough content against help articles
 * Returns any discrepancies or missing content
 */
export function validateWalkthroughs(): { valid: boolean; issues: string[] } {
  const issues: string[] = [];
...
// Export for use in build scripts or manual execution
export default {
  updateWalkthroughs,
  watchAndUpdateWalkthroughs,
