...
-- Function that checks if all scores for a sub-event are certified
-- and calls the notification edge function via pg_net
...
BEGIN
  ...
  IF total_count > 0 AND uncertified_count = 0 THEN
    func_url := rtrim(current_setting('app.settings.supabase_url', true), '/') || '/functions/v1/notify-certification-complete';
    anon_key := current_setting('app.settings.supabase_anon_key', true);

    -- If settings not available, construct from env
...
-- Attach trigger to judge_scores table
DROP TRIGGER IF EXISTS trg_notify_full_certification ON public.judge_scores;
CREATE TRIGGER trg_notify_full_certification
  AFTER UPDATE OF is_certified ON public.judge_scores
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_on_full_certification();
